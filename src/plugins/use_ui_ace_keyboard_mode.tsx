...
const OverlayText = () => (
  <>
    <p>Press Enter to start editing.</p>
    <p>When you&rsquo;re done, press Escape to stop editing.</p>
  </>
);
...
export function useUIAceKeyboardMode(
  ...
  useEffect(() => {
    ...
    const isAutoCompleteVisible = () => {
      const autoCompleter = document.querySelector<HTMLDivElement>(
        ".ace_autocomplete"
      );
      if (!autoCompleter) {
        return false;
    ...
    if (aceTextAreaElement) {
      ...
      aceTextAreaElement.parentElement!.insertBefore(
        overlayMountNode.current,
        aceTextAreaElement
      );
    ...
    return () => {
      if (aceTextAreaElement) {
        ...
        if (
          textAreaContainer &&
          textAreaContainer.contains(overlayMountNode.current!)
        ) {
