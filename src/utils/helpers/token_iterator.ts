import { CoreEditor, Position, Token, TokensProvider } from "../../types";
...
function _isColumnInTokenRange(column: number, token: Token) {
...
export class TokenIterator {
  ...
  constructor(
    ...
    const tokenIdx = this.tokensLineCache.findIndex((token) =>
      _isColumnInTokenRange(startPosition.column, token)
    );
    if (tokenIdx > -1) {
      this.updatePosition({
        tokenIdx,
